...
def activity_planner(use_thread_lock: bool = True):
    ...
    def find_activities_executable(job_id: int, search_query: str) -> Tuple[FunctionResultStatus, str, dict]:
        ...
        activities = IInventory(
            jobId=job_id,
            type="object",
            value=json.dumps(activities_data[:5]),
            clientName=job.get("clientName"),
            providerName=job.get("providerName"),
        )

        acp_plugin.add_produce_item(activities)
    ...
    find_activities = Function(
        ...
        args=[
            ...
            Argument(
                name="search_query",
                type="str",
                description="Search query for relevant activities. Should always start with 'Events in {location}, {following details}",
            )
        ],
